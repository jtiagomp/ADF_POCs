    ...
    if(AdfPage.PAGE.isSynchronizedWithServer())
    {
      ...
      if (window.matchMedia) // supported only on modern browsers
      {
        ...
        var func = function ()
        {
          ...
          var initialMatch = compMQs[mediaQueryString].mqList.matches;
          // initial check
          if (((currentLayout != initialMatchValue) && !initialMatch) ||
              ((currentLayout == initialMatchValue) && initialMatch))
          {
            changeLayout(initialMatch, contentContainer, compMQs[mediaQueryString])
          }
        }
